...
#define uipINIT_WAIT    ( 100000 / portTICK_RATE_US )
...
struct timer periodic_timer, arp_timer, iotimer;
extern void ( vEMAC_ISR_Wrapper )( void );
	...
	timer_set( &iotimer, CLOCK_CONF_SECOND / 100 );
	uip_init();
	...
	vTaskDelay( 2*configTICK_RATE_HZ ); /* Give a chance for the FPGA to initialize */
	...
	for( ;; )
	{
		...
		if( ( uip_len > 0 ) && ( uip_buf != NULL ) )
		{
			...
			if ( xHeader->type == htons( 0x8086 ) ) {
				etherio_in();
			}
			else if( xHeader->type == htons( UIP_ETHTYPE_IP ) )
			{
				uip_arp_ipin();
		...
		{
			if( timer_expired( &iotimer ) && ( uip_buf != NULL ) ) {
				/* EtherIO master mode timer */
#warning "DAC to FPGA by calling send_response"
			}
