...
#define emacWAIT_FOR_LINK_TO_ESTABLISH ( 500000 / portTICK_RATE_US )

/* Short delay used in several places during the initialisation process. */
#define emacSHORT_DELAY				   ( 5 )

/* Hardware specific bit definitions. */
#define emacLINK_ESTABLISHED		( 0x0004 )
#define emacAUTONEG_COMPLETED		( 0x0020 )

#define emacFULL_DUPLEX_ENABLED		( 0x0140 ) /* 10/100 Full duplex mask (either bit will be set) */
#define emac100BASE_T_MODE			( 0x0180 ) /* 100 Full / 100 Half */


#define emacPINSEL2_VALUE 0x50150105

/* If no buffers are available, then wait this long before looking again.... */
#define emacBUFFER_WAIT_DELAY	( 3000 / portTICK_RATE_US )
 ...
 * parameters.
 */
static long prvSetupLinkStatus( unsigned phy_id );
...
unsigned phy_in_use = 0;
	...
	if( lReturn == pdPASS )
	{
		lReturn = prvSetupLinkStatus(phy_in_use);
	}
...
unsigned short usLinkStatus, usAutoNegStatus;
//unsigned phy_linkstatus_reg, phylinkEstablished_mask;

#if 0
    if (phy_in_use == DP83848C_ID) {
      phy_linkstatus_reg = PHY_REG_STS;		// Default to DP83848C
      phy_linkEstablished_mask = 0x0001;
    }
#endif
    ...
    if (phy_id == LAN8720_ID) {
      ...
      return(pdFAIL);  // We don't have a valid phy to work with
    }
	...
	if( lReturn == pdPASS ) {
      lReturn = pdFAIL;  // Pessimistic
	  prvWritePHY(PHY_REG_BMCR, PHY_AUTO_NEG);
	  for (x = 0; x < 10; x++)
	  {
	      usLinkStatus = prvReadPHY(PHY_REG_BMSR, &lReturn );
	      if (usLinkStatus & emacAUTONEG_COMPLETED ) {
	...
	if( lReturn == pdPASS )
	{
		usAutoNegStatus = prvReadPHY( PHY_REG_ANLPAR, &lReturn );
		/* Configure Full/Half Duplex mode. */
		if( usAutoNegStatus & emacFULL_DUPLEX_ENABLED )
		{
		...
		if( usAutoNegStatus & emac100BASE_T_MODE )
		{
			/* 100MBit mode. */
...
unsigned long ulAttempts = 0UL;
	...
	NULL. */
	while( TX_DESC_PACKET( LPC_EMAC->TxProduceIndex ) != ( unsigned long ) NULL )
	{
	...
	LPC_EMAC->TxProduceIndex = ( LPC_EMAC->TxProduceIndex + 1 ) % NUM_TX_FRAG;
...
long lHigherPriorityTaskWoken = pdFALSE;
	...
	if( ulStatus & INT_TX_DONE )
	{
		...
		//{
			...
			prvReturnBuffer( ( unsigned char * ) TX_DESC_PACKET( LPC_EMAC->TxConsumeIndex ) );
            TX_DESC_PACKET( LPC_EMAC->TxConsumeIndex  ) = ( unsigned long ) NULL;
		//}
	}
